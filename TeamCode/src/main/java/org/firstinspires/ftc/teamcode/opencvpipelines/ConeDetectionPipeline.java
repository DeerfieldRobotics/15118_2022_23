    ...
    public Mat processFrame(Mat input) {
        ...
        for(int i = 0; i<initial.length;i++) { //collects all cr cb values values
            for(int j = 0; j<initial[0].length;j++) {
                initial[i][j][0]=Core.sumElems(workingMatrix.submat(new Range(j*1920/initial[0].length,(j+1)*1920/initial[0].length),new Range(i*1080/initial.length,(i+1)*1080/initial.length))).val[1]; //finds sum of submat and gets cb value
                cbTotal+=initial[i][j][0];
                initial[i][j][1]=Core.sumElems(workingMatrix.submat(new Range(j*1920/initial[0].length,(j+1)*1920/initial[0].length),new Range(i*1080/initial.length,(i+1)*1080/initial.length))).val[2]; //finds sum of submat and gets cr value
